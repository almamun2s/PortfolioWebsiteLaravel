    ...
    <ul class="navbar-nav ml-auto">
        ...
        <li class="nav-item dropdown no-arrow mx-1">
            <a class="nav-link dropdown-toggle" href="#" id="messagesDropdown" role="button"
                ...
                <!-- Counter - Messages -->
                @php
                    $messageCount = auth()->user()->unreadNotifications()->count();
                @endphp
                @if ($messageCount > 0)
            ...
            <div class="dropdown-list dropdown-menu dropdown-menu-right shadow animated--grow-in"
                ...
                @forelse (auth()->user()->notifications as $notification)
                    <a class="dropdown-item d-flex align-items-center"
                        href="{{ route('admin.contact.readNotify', $notification->id) }}">
                        <div class=" {{ $notification->read_at ? '' : 'font-weight-bold' }} ">
                            <div class="text-truncate">{{ $notification->data['message'] }}</div>
                            <div class="small text-gray-500">{{ $notification->created_at->diffForHumans() }}</div>
                        </div>
                    </a>
                @empty
                    <span class="p-3" style="width:100%;display:inline-block;text-align: center;">No Messages to
                        show</span>
                @endforelse
